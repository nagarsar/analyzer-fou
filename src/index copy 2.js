...
import { Table, Icon, Segment, Grid, Divider, Feed, Image, Progress, Statistic} from "semantic-ui-react";
...
class PlaylistRow extends React.Component {
  ...
  render() {
    ...
    console.log("appel PlaylistRow :", playlist.id)
...
class LastFmBiography extends React.Component {
  ...
  getBiography = () => {
    const apiBiography= 'http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist='+this.props.lastfm_query+'&api_key=5f1ef2dc332e60a42232196316f22b3e&format=json'
    fetch(apiBiography)
      .then(result => result.json() )
      .then(data =>{
        console.log(data)
            ...
            <Grid columns={2}>
              <Grid.Column>
                <h5>Comments</h5>
                 <div class="scrollable" tabIndex="0">
                  <div class="scrollable-content">
                  {comments.map(comment => <YoutubeComments key={comment.authorChannelUrl} user={comment}/> )}
                  </div>
                </div>
              </Grid.Column>
              <Grid.Column>
